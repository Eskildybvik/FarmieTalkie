...
class TimerLogic:
	...
	def __init__(self, name, duration, component):
		...
		t0 = {
			"source": "initial",
			"target": "running",
			"effect": f"start_timer('t', {self.duration*1000})"
		}
		t1 = {
			"source": "running",
			"target": "finished",
	...
	def stop(self):
		self.component.mqtt_client.publish(MQTT_TOPIC_OUTPUT, "{'answer': 'finished_timer', 'name': '" + self.name + "'}")
	
	def send_status(self): 
		time_left = self.stm.get_timer('t') // 1000
		self.component.mqtt_client.publish(MQTT_TOPIC_OUTPUT, 
			"{'answer': 'status_timer', 'name': '" + self.name + "', 'time_left': '" + str(time_left) + "'}"
		)
	...
	def on_message(self, client, userdata, msg: mqtt.MQTTMessage):
		...
		* throw the message away.
		...
		if command == "cancel_timer": 
			self.stm_driver.send("stop", payload["name"])
		elif command == "new_timer":
			new_timer_machine = TimerLogic(name=payload["name"], duration=payload["duration"], component=self)
			self.stm_driver.add_machine(new_timer_machine.stm)
		elif command == "status_single_timer":
			self.stm_driver.send("status", payload["name"])
		elif command == "status_all_timers":
			name_time_pairs = []
			for name, stm in self.stm_driver._stms_by_id.items():
				if stm.get_timer("t") == None:
					continue
				name_time_pairs.append({
					"name": name,
					"time_left": stm.get_timer("t") // 1000
				})
			pl = json.dumps({
				"answer": "get_all_timers",
				"timers": name_time_pairs
			})
			self.mqtt_client.publish(MQTT_TOPIC_OUTPUT, pl)
